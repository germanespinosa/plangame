let gameStatus = {
    ...
    showMenu: function (p){
        ...
        gameStatus.mapSpinner = new Spinner(game.width * .1,game.height * .48,game.width *.525,game.height *.05,options,"8bit", groups.status);
        gameStatus.mapSpinner.selected = gameStatus.spinnersValues[0];
        gameStatus.mapSpinner.update();
        gameStatus.modSpinner = new Spinner(game.width * .1,game.height * .667,game.width *.525,game.height *.05,["easy","natural", "hard"],"8bit", groups.status);
