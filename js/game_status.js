let gameStatus = {
    ...
    showMenu: function (p){
        ...
        gameStatus.showMessage("DIFFICULTY: ",{w:.8,h:.03},0xFFFFFF,{x:.025,y:.64}, {x:0,y:.5});
        ...
        gameStatus.mapSpinner = new Spinner(game.width * .1,game.height * .48,game.width *.525,game.height *.0667,options,"8bit", groups.status);
        gameStatus.mapSpinner.selected = gameStatus.spinnersValues[0];
        gameStatus.mapSpinner.update();
        gameStatus.modSpinner = new Spinner(game.width * .1,game.height * .667,game.width *.525,game.height *.0667,["easy","natural", "hard"],"8bit", groups.status);
